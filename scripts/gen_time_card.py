#!/usr/bin/env python3
from __future__ import annotations

from datetime import datetime
from zoneinfo import ZoneInfo
from pathlib import Path

TZ = "America/New_York"
OUT = Path("assets/time-card.svg")

def pick_theme(hour: int):
    # Auto color by time of day
    if 5 <= hour < 12:   # morning
        return ("#0ea5e9", "#22c55e")  # sky -> green
    if 12 <= hour < 17:  # afternoon
        return ("#fb923c", "#f43f5e")  # orange -> pink
    if 17 <= hour < 21:  # evening
        return ("#a855f7", "#6366f1")  # purple -> indigo
    # night
    return ("#0f172a", "#1d4ed8")      # slate -> blue

def esc(s: str) -> str:
    return (s.replace("&", "&amp;")
             .replace("<", "&lt;")
             .replace(">", "&gt;")
             .replace('"', "&quot;"))

def main():
    now = datetime.now(ZoneInfo(TZ))

    # Time like "11:11" + "PM"
    t_main = now.strftime("%-I:%M") if hasattr(now, "strftime") else now.strftime("%I:%M").lstrip("0")
    t_ampm = now.strftime("%p")

    # Date like "Wednesday, June 15th"
    weekday = now.strftime("%A")
    month = now.strftime("%B")
    day = int(now.strftime("%d"))
    year = now.strftime("%Y")

    # 1st/2nd/3rd/th suffix
    if 11 <= day % 100 <= 13:
        suffix = "th"
    else:
        suffix = {1: "st", 2: "nd", 3: "rd"}.get(day % 10, "th")

    date_line = f"{weekday}, {month} {day}{suffix}"
    # If you also want year, use: f"{weekday}, {month} {day}{suffix}, {year}"

    c1, c2 = pick_theme(now.hour)

    svg = f"""<svg xmlns="http://www.w3.org/2000/svg" width="560" height="300" viewBox="0 0 560 300" role="img" aria-label="Time card">
  <defs>
    <linearGradient id="bg" x1="0" y1="0" x2="1" y2="0">
      <stop offset="0%" stop-color="{c1}"/>
      <stop offset="100%" stop-color="{c2}"/>
    </linearGradient>
    <filter id="shadow" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0" dy="14" stdDeviation="18" flood-color="#000" flood-opacity="0.55"/>
    </filter>
  </defs>

  <!-- Card -->
  <rect x="40" y="45" rx="28" ry="28" width="480" height="210" fill="url(#bg)" filter="url(#shadow)"/>

  <!-- Subtle gloss -->
  <path d="M68 75 C180 20, 380 20, 492 75 L492 105 C380 70, 180 70, 68 105 Z"
        fill="#fff" opacity="0.12"/>

  <!-- Time -->
  <text x="88" y="155" fill="#fff"
        font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial"
        font-size="96" font-weight="800" letter-spacing="-2">{esc(t_main)}</text>

  <text x="320" y="155" fill="#fff"
        font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial"
        font-size="28" font-weight="700" opacity="0.92">{esc(t_ampm)}</text>

  <!-- Date -->
  <text x="92" y="200" fill="#fff"
        font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial"
        font-size="28" font-weight="600" opacity="0.95">{esc(date_line)}</text>

  <!-- Moon icon -->
  <g transform="translate(458,78) scale(1.35)" fill="#fff" opacity="0.95">
    <path d="M6 .278a.768.768 0 0 1 .08.858 7.208 7.208 0 0 0-.878 3.46c0 4.021 3.278 7.277 7.318 7.277.527 0 1.04-.055 1.533-.16a.787.787 0 0 1 .81.316.733.733 0 0 1-.031.893A8.349 8.349 0 0 1 8.344 16C3.734 16 0 12.286 0 7.71 0 4.266 2.114 1.312 5.124.06A.752.752 0 0 1 6 .278z"/>
    <path d="M10.794 3.148a.217.217 0 0 1 .412 0l.387 1.162c.173.518.579.924 1.097 1.097l1.162.387a.217.217 0 0 1 0 .412l-1.162.387a1.734 1.734 0 0 0-1.097 1.097l-.387 1.162a.217.217 0 0 1-.412 0l-.387-1.162A1.734 1.734 0 0 0 9.31 6.593l-1.162-.387a.217.217 0 0 1 0-.412l1.162-.387a1.734 1.734 0 0 0 1.097-1.097l.387-1.162z"/>
  </g>

  <!-- Tiny footer -->
  <text x="92" y="235" fill="#fff" opacity="0.70"
        font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial"
        font-size="14">Auto-updated • {esc(TZ)}</text>
</svg>
"""
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(svg, encoding="utf-8")
    print(f"Wrote {OUT}")

if __name__ == "__main__":
    main()
